---
title: "Hands-On Exercise 4a:</br>Visualising Distribution"
...
Two relatively new statistical graphic methods for visualising distribution, namely ridgeline plot and raincloud plot by using ggplot2 and its extensions.
...
There are several ways to plot ridgeline plot with R. In this section, to plot ridgeline plot by using [ggridges](https://wilkelab.org/ggridges/index.html) package.
...
Instead of using number to define the quantiles, specify quantiles by cut points such as 2.5% and 97.5% tails to colour the ridgeline plot as shown in the figure below.
...
In this section, to create a raincloud plot to visualise the distribution of English score by race. It will be created by using functions provided by **ggdist** and ggplot2 packages.
...
Next, add the second geometry layer using [`geom_boxplot()`](https://r4va.netlify.app/chap09) of ggplot2. This produces a narrow boxplot. Reduce the width and adjust the opacity.
...
Next, add the third geometry layer using [`stat_dots()`](https://mjskay.github.io/ggdist/reference/stat_dots.html) of ggdist package. This produces a half-dotplot, which is similar to a histogram that indicates the number of samples (number of dots) in each bin. Select side = “left” to indicate position on the left-hand side.
